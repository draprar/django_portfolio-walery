...
Block analysis using spaCy (pl_core_news_sm):
 - detects content types: people, organizations, dates, numbers, locations, units
 - evaluates semantic significance of a change (semantic_score)
 - classifies change type: substantive, editorial, formal, technical
 - optionally groups changes thematically (AI clustering)
...
def extract_labels_spacy(text: str) -> List[str]:
    ...
    if re.search(r"(kg|mm|cm|km|\bm\b|%)", text):
        labels.add("unit")
...
def analyze_change(block: Dict[str, Any]) -> Dict[str, Any]:
    """
    Main AI function.
    NOTE: tables are intentionally excluded from AI analysis.
    """

    # Tables are not analyzed semantically
    if block.get("old", {}).get("type") == "table":
        return {
            "labels": [],
            "semantic_score": None,
            "change_type": "table_change",
            "confidence": 1.0,
        }
    ...
    merged_text = f"{old_text} {new_text}".strip()
    ...
    # 4. confidence
    conf = 1.0 - abs(0.85 - sim) if sim is not None else 0.5
...
def cluster_changes(blocks: List[Dict[str, Any]]) -> Dict[int, List[int]]:
    ...
    for idx, b in enumerate(changed_blocks):
    ...
    for vec_idx, lbl in enumerate(kmeans.labels_):
        clusters.setdefault(int(lbl), []).append(indices[vec_idx])
